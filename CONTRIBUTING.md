...
`cloud-itonami-isco-9613` accepts contributions to the OSS actor, policy tests,
...
- Do not commit real worker, route or operator data, credentials or operating
  documents.
- Keep production writes and disclosures behind StreetCleaningGovernor.
...
- Never widen the closed op-allowlist to include a cleaning-work-
  execution op (e.g. one that would authorize a specific sweeping/
  cleaning operation to proceed), a route-safety-clearance op (e.g. one
  that would declare a route cleared for safety), or a

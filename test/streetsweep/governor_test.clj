...
(deftest hard-on-scope-excluded-op-authorize-sweeping-operation
  (testing "authorizing a cleaning-work-execution decision (a specific sweeping operation) is a permanent block, never a routine op"
    (let [st (fresh-store)
          v (governor/check req {} (assoc (op :authorize-sweeping-operation) :confidence 0.99) st)]
      (is (:hard? v))
      (is (some #(= :scope-excluded-action (:rule %)) (:violations v))))))

(deftest hard-on-scope-excluded-op-authorize-cleaning-dispatch
  (testing "authorizing a cleaning dispatch is a permanent block, never a routine op"
    (let [st (fresh-store)
          v (governor/check req {} (assoc (op :authorize-cleaning-dispatch) :confidence 0.99) st)]
      (is (:hard? v))
      (is (some #(= :scope-excluded-action (:rule %)) (:violations v))))))
...
(deftest hard-on-scope-excluded-rationale-authorize-sweeping-operation
  (testing "defense-in-depth: a rationale that itself attempts to authorize the sweeping operation to proceed is blocked even if the op looks routine"
    (let [st (fresh-store)
          v (governor/check req {} (assoc (op :schedule-crew-operation)
                                           :rationale "recommend we authorize the sweeping operation to proceed now")
                             st)]
      (is (:hard? v))
      (is (some #(= :scope-excluded-action (:rule %)) (:violations v))))))
...
(deftest always-escalates-safety-concern-even-at-high-confidence
  (testing "a vehicle-traffic-hazard/weather-terrain-exposure-hazard concern always requires human sign-off"
    (let [st (fresh-store)
...
(deftest always-escalates-weather-terrain-safety-concern-even-at-high-confidence
  (testing "a weather/terrain-exposure hazard concern also always requires human sign-off (the second, independent hazard-scope dimension for this occupation)"
    (let [st (fresh-store)
          v (governor/check req {} (assoc (op :flag-safety-concern :hazard-type :weather-terrain-exposure-hazard)
                                           :confidence 0.99)
                             st)]
      (is (not (:hard? v)))
      (is (:escalate? v)))))
...
(deftest default-mock-advisor-proposals-never-self-trip-on-scope-exclusion
  (testing "the governor's scope-exclusion term list must never match the mock advisor's own default rationale text for any allowlisted op — CLAUDE.md's known self-tripping bug pattern (rationale legitimately contains bare nouns like 'street'/'sweeping'/'broom'/'debris'/'route'/'route safety supervisor', but never the full finalization-action phrases)"
    (let [st (fresh-store)
      ...
      (doseq [o ops]
        (let [request {:worker-id "worker-1" :op o :route-id "ROUTE-1"
                        :stake :low :task "routine street-sweeping and site-cleaning task"
